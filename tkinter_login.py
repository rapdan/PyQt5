...
root = Tk()
root.geometry('800x280+1000+300')
...
l_user = Label(root, text="Login:").grid(
...
e_pass = Entry(root, show='*').grid(row=1, column=1,
                                    columnspan=2, padx=10, sticky=W+E)

btn_login = Button(root, text="OK", padx=30).grid(
    row=2, column=1, padx=10, pady=10, sticky=W)
btn_reg = Button(root, text="Rejestracja", padx=10).grid(row=3, column=1)
btn_forgot = Button(root, text="Zapomniane hasło?",
                    padx=10).grid(row=3, column=2, padx=10, pady=20)
